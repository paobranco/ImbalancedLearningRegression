...
def borderline_svm_smote(
    ...
    train_X = normalized_data[normalized_data.columns[:(d-1)]]
    train_Y = normalized_data[d-1]
    svr = SVR(kernel="rbf", gamma="scale", epsilon=0.2)
    ...
    for i in range(n_bumps):
        ...
        if s_perc[i] > 1:
            ...
            synth_obs = over_sampling_smote(
                data = data,
                index = list(set(svr.support_) & set(b_index[i].index)),
                perc = s_perc[i],
                k = k
            )
