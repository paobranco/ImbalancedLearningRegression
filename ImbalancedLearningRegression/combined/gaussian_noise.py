...
class GaussianNoise(BaseCombinedSampler):
    ...
    def __init__(self, drop_na_row: bool = True, drop_na_col: bool = True, samp_method: SAMPLE_METHOD = SAMPLE_METHOD.BALANCE, 
                 ...
                 pert: int | float = 0.02, replace: bool = False, manual_perc: bool = False, perc_undersampling: float | int = -1, 
                 perc_oversampling: float | int = -1) -> None:
        ...
        for idx, pts in indices.items():
            ...
            if perc[idx] < 1:
                if self.under_samp is True:
                    ...
                    chosen_index = random.choice(  # modified
                        a = list(pts.index), 
