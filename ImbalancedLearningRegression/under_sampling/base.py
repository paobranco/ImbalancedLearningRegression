...
class BaseUnderSampler(BaseSampler):
    """Base class for all undersampler algorithms.
    ...
    def fit_resample(self, data: DataFrame, response_variable: str) -> DataFrame:
        ...
        """
        raise NotImplementedError("BaseUnderSampler must never call fit_resample as it's just a base abstract class.")

    @abstractmethod
    def _undersample(self, data: DataFrame, indices: dict[int, "Series[Any]"], perc: list[float]) -> DataFrame:
        """Undersample the data set for an undersampler algorithm.
        ...
        -------
        new_data: DataFrame
            DataFrame that contains the undersampled data set.

        """
        raise NotImplementedError("BaseUnderSampler must never call _undersample as it's just a base abstract class.")